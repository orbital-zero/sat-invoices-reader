...
class CSVReader:
    ...
    def read(self, path: str, _type: str):
        ...
        if _type == 'D':
            ...
            self.__read_files_to_csv(
                path, self.deductionParser, read_invoice, entries)
            ...
            self.__read_files_to_csv(
                path, self.payrollParser, read_invoice, entries)
    ...
    def __read_files_to_csv(
            self, path: str, invoice_parser: InvoiceParserInterface, read_invoice, entries):
    ...
    def __read_invoices(self, path: str, tree_parser,
                        invoice_parser: InvoiceParserInterface, read_invoice):
